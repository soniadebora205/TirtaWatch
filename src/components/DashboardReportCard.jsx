  ...
  return [
    {
      ...
      dotColor: "bg-[#22c55e]", // Hijau karena pasti sudah dilapor
      opacity: "opacity-100",
    },
    {
      id: 2,
      title: "Diverifikasi & Diproses",
      time: isProses ? formatDate(report.updated_at) : "Menunggu verifikasi...",
      dotColor: isProses ? "bg-[#22c55e]" : "bg-[#0ea5e9]",
    ...
    },
    {
      ...
      time: isSelesai ? formatDate(report.finished_at || report.updated_at) : "-",
      dotColor: isSelesai ? "bg-[#22c55e]" : "bg-[#0ea5e9]",
      opacity: isSelesai ? "opacity-100" : "opacity-50",
    },
  ];
}

// PERHATIKAN: Sekarang kita menerima props { report }, bukan { laporan }
export default function DashboardReportCard({ report }) {
  // Panggil fungsi timeline menggunakan data API asli
  ...
  return (
    <div className="bg-[#f0f8ff] rounded-3xl p-7 flex flex-col lg:flex-row gap-8 mb-6 border border-sky-100 shadow-sm transition hover:shadow-md">
      ...
      <div className="flex-1">
        {/* Badge Kategori */}
        <div className="inline-block px-3 py-1 bg-white border border-red-100 text-red-500 text-[11px] font-bold rounded-full mb-3 uppercase tracking-wider">
          {report.category_key || report.categoryLabel || "Laporan"}
        </div>
        ...
        <div className="relative pl-3 border-l-2 border-slate-300 space-y-8">
          {timelineSteps.map((step) => (
            <div key={step.id} className={`relative ${step.opacity}`}>
              {/* Titik / Dot */}
              <div className={`absolute -left-[21px] top-1 w-5 h-5 rounded-full border-4 border-[#f0f8ff] ${step.dotColor}`} />
      ...
      <div className="w-full lg:w-[400px] flex flex-col gap-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-gray-200 rounded-2xl aspect-square w-full relative overflow-hidden">
               <Image 
                src={report.after_image_url || "/placeholder-after.jpg"} 
                alt="Bukti Sesudah" 
                fill
                className="object-cover"
              />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-white p-3 rounded-xl border border-sky-100 shadow-sm">
                ...
                <div className="text-[12px] font-semibold text-[#1a2b4c] mt-1 leading-snug">
                  {report.estimated_finish ? formatDate(report.estimated_finish) : "-"}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border border-sky-100 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <div className="flex gap-2 text-xl cursor-pointer opacity-70">
                  💬 📸 🎵 ⋯
                </div>
              </div>
